...
function ExperienceQuestions({ email, onComplete }) {
  ...
  const handleSubmit = async () => {
    ...
    // Check email
    if (!email) {
      setError("Email is missing. Please try joining again.");
      return;
    }
    ...
    try {
      const response = await fetch(
        "https://indiquant-waitlist-backend.onrender.com/api/waitlist/experience",
        {
          ...
          body: JSON.stringify({
            // IMPORTANT:
            // use the email prop, NOT user.email
            email: email,
      ...
      // Experience questions completed
      // Go back to SuccessCard
      onComplete();

    } catch (err) {
      console.error(
        "Experience submission error:",
        err
      );
